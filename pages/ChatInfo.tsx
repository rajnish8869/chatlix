      ...
      <div className="flex-1 overflow-y-auto pb-10" style={{ paddingBottom: 'calc(2.5rem + env(safe-area-inset-bottom))' }}>
          ...
          <div className="flex flex-col items-center pt-8 pb-10">
             <div className="mb-4 relative">
                 <Avatar name={currentChat.name || "C"} size="xl" className="shadow-2xl shadow-primary/20" showStatus={false} />
             </div>
             <h2 className="text-3xl font-extrabold tracking-tight text-center px-4">{currentChat.name || "Conversation"}</h2>
             <span className="mt-2 px-3 py-1 bg-surface-highlight rounded-full text-xs font-bold text-text-sub">
                 {currentChat.type === 'group' ? 'Group Chat' : 'Private Conversation'}
             </span>
          </div>

          <div className="px-5">
              <h3 className="text-xs font-bold text-text-sub uppercase tracking-wider mb-4 ml-2">Participants ({participants.length})</h3>
              <div className="bg-surface rounded-[24px] border border-white/5 overflow-hidden shadow-soft">
                  {participants.map((p: any, idx) => (
                      <div 
                        key={p.user_id}
                        className={`flex items-center gap-4 p-4 ${idx !== participants.length - 1 ? 'border-b border-border' : ''}`}
                      >
                          <Avatar name={p.username} size="md" online={p.status === 'online'} />
                          <div className="flex-1">
                              <div className="flex items-center gap-2">
                                <h4 className="font-bold text-text-main text-base">{p.username}</h4>
                                {p.isMe && <span className="text-[10px] bg-primary/20 text-primary px-2 py-0.5 rounded-full font-bold">YOU</span>}
                              </div>
                              <p className="text-xs text-text-sub opacity-70">{p.email || p.status}</p>
                          </div>
                      </div>
                  ))}
              </div>
